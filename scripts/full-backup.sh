#!/bin/bash
# Creates a self-contained ZIP backup of the entire Games Signup app + database
...
ZIP_NAME="GamesSignup_v${VERSION}_${TIMESTAMP}.zip"
ZIP_PATH="${BACKUP_DIR}/${ZIP_NAME}"
TEMP_DIR=$(mktemp -d)

echo "=== Games Signup Full Backup ==="
...
turso db shell games-signup .dump | sqlite3 "$DB_FILE"
...
# Games Signup Environment Configuration
...
# Games Signup — Installation Guide
...
4. Set up your database:
   ...
   - Option B (Restore from backup): Create a new Turso database and import `database.sqlite`:
     ```
     turso db create games-signup
     turso db shell games-signup < database.sqlite
     ```
...
PROJECT_DIR="$(pwd)"
FINAL_DIR="${TEMP_DIR}/GamesSignup"
mkdir -p "$FINAL_DIR"
...
(cd "${TEMP_DIR}" && zip -r -q "${PROJECT_DIR}/${ZIP_PATH}" "GamesSignup")
...
ls -1t "$BACKUP_DIR"/GamesSignup_*.zip 2>/dev/null | head -5
